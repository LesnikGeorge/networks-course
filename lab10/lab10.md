...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - Весь фрейм 72 байта, IP-пакет 56 байтов, в IP-заголовке 20 байт, UPD-пакет 36 байтов, дата 28 байт
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      * Не изменяются
         ...
         - Флаги, Отступ фрагмента, протокол, статус чек суммы, адреса источника и конечного пункта, индекс потока
      * Должны не изменяться все описаные выше
      * Какие поля меняются
         - Идентификатор
         - Время жизни
         - Чек сумма
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Идентификатор изменяется случайно, закономерности не заметил
      - ![alt text](image-4.png)
      - идентификаторы этих запросов последовательно: 0x3086, 0x2040, 0x1f88, 0x112b, 0xa5ba, 0x5153, 0xbedd и тд
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification изменяется, для каждого сообщения он свой, TTL неизменени, он стандартный 64
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - да, создано было 3 фрагмента
      - [3 IPv4 Fragments (3480 bytes): #183(1480), #184(1480), #185(520)]
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Identification не изменился, зато изменялись Fragment Offset. для трёх пакетов они равны 0, 1480, 2960
      - Так же очевидно, от пакета к пакету менялась чексумма.
