...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Сервер взял номер в порядковом номере и добавил 1, релультат закинул в ответный номер. 
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Скрин был первым, у него 1 порядковый номер
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   пришлось табличку сделать

   | Сегмент | № пак. (Send) | Seq | Время отправки | № пак. ACK | Время ответа | RTT (сек)|
   | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
   | 1 (POST) | 350 | 1 | 7.798989508 | 378 | 7.937256661 | 0.138267153 |   
   | 2 | 351 | 612 | 7.799165996 | 379 | 7.937258966 | 0.138092970 |
   | 3 | 352 | 2040 | 7.799174098 | 380 | 7.937259944 | 0.138085846 |
   | 4 | 356 | 3468 | 7.806074863 | 383 | 7.937800512 | 0.131725649 |
   | 5 | 357 | 4896 | 7.806123612 | 384 | 7.937801909 | 0.131678297 |
   | 6 | 358 | 6324 | 7.808153257 | 385 | 7.939325208 | 0.131171951 |
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - мы отправили 152 932 байта
   - с пакет №350 в 7.798989508 до пакет №590 в 8.393146823, разница 0.594157 секунд
   $$R_{мой} = \frac{152932 \text{ байт}}{0.594157 \text{ сек}} \approx 257\,393 \text{ байт/сек}$$
   $$\approx 2059 \text{ Кбит/сек} \approx 2.06 \text{ Мбит/сек}$$
...
#### Решение
- Время передачи: $d_{\text{пер}} = \frac{L}{R} = \frac{1500 * 8}{10^9} = 0.000012 \text{ с} = 0.012 \text{ мс}$
- Коэф. использования канала для протокола с окном $n$: $U = \frac{n \cdot d_{\text{пер}}}{d_{\text{пер}} + RTT}$
- Нам нужно, $U > 0.98 \Rightarrow 0.98 < \frac{n \cdot 0.012}{0.012 + 30} = 0.98 < \frac{n \cdot 0.012}{30.012}$
- $n > \frac{0.98 \cdot 30.012}{0.012} = 0.98 \cdot 2501 = 2450.98$

**Ответ:** $n = 2451$ пакетов. 
