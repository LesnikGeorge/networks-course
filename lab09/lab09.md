...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   ...
   - Source Address: 192.168.31.217 - мой
   - Destination Address: 104.71.149.245 - MIT
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - ![alt text](image-2.png)
   - ICMP является протоколом сетевого уровня. Он не использует транспортный уровень, в его заголовках нет полей для портов, у него тип и код.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - ещё 3 поля, каждое по 2 байта, в сумме 6
   - Checksum
   - Identifier 
   - Sequence Number
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Code: 0
   - всё так же, 2 байта на каждое из 3, 6 байт в сумме
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - тип и код совпадат, отличается только время жизни и очевидно чексумма, порядковый номер и индификатор
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - В этих полях записан изначальный пинг запрос от меня
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Type: Echo (ping) reply (0) - тип 0, а не 11
   - Есть полезная нагрузка
   - Разница в том, что этот пакет ответ MIT хоста. Мы успешно дошли до него с TTL 10 (успели отправить с TTL 11, так как ответ долго шёл) и он нам стандартно ответил.
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Да, есть, между 3 и 5, есть прыжок с ~13 до ~24 мс через не публичный маршрутизатор, в среднем 5.5 мс. Так как до MIT пакет дошёл в среднем за 24 мс через 10 прыжков - 2.4 мс в среднем.
   - ...
   - И я понял, что облажался, запрошенная задержка скорее всего проход по тихоокеанскому кабелю, меня же просили хост на другом континенте, я запустил тресроут с айпи MIT, что дал пинг, ведь при пинге, была задерка в 280 мс
   - ![alt text](image-9.png)
   - И да, тут прям видно с 6 по 7 прыжок с ~50 мс до ~220
   - У маршрутизаторов нет имён, whois тоже не даёт результатов. Так что не могу определить местоположение
   - ![alt text](image-10.png)
   - (Этот маршрутизатор точно не в амстердаме, кажется это где RIPE NCC)
